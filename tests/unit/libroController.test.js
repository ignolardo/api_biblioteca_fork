...
const { find, findById, create, findByIdAndUpdate, findByIdAndRemove } = require("../../src/models/libroModel");
...
describe("Libro Controller", () => {
  ...
  test("getLibros debería obtener todos los libros", async () => {
    ...
    find.mockResolvedValue(mockLibros);
    const mockReq = {};
  ...
  test("getLibroById debería obtener un libro", async () => {
    ...
    findById.mockResolvedValue(mockLibro);
  ...
  test("createLibro debería crear un nuevo libro", async () => {
    ...
    mockLibro.save = () => {};
    create.mockResolvedValue(mockLibro);
  ...
  test("updateLibro debería actualizar un libro existente", async () => {
    ...
    findByIdAndUpdate.mockResolvedValue(libroActualizadoMock);
  ...
  test("deleteLibro debería eliminar un libro existente", async () => {
    ...
    findByIdAndRemove.mockResolvedValue(mockLibroEliminado);
    const mockReq = { params: { id: "1" } };
    await deleteLibro(mockReq, mockRes);
    expect(findByIdAndRemove).toHaveBeenCalledWith(
      mockReq.params.id
    );
    expect(mockRes.status).toHaveBeenCalledWith(200);
